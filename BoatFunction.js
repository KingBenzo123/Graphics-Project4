import { vec4, mat4, flatten, perspective, translate, lookAt, rotateY, rotateZ, initFileShaders, vec3 } from './helperfunctions.js';
// ** Global Declarations **
let gl;
let canvas;
let program;
...
let vColor;
let vNormal;
let vShininess, ambLight;
let spotlightPos, spotlightColor, spotlightDirection;
let redLightPos, redLightColor;
let greenLightPos, greenLightColor;
let whiteLightPos, whiteLightColor;
let orangeLight1Pos, orangeLight1Color, orangeLight1Direction;
let orangeLight2Pos, orangeLight2Color, orangeLight2Direction;
let SPOTLIGHT, FLASHING, time;
/**
...
window.onload = function init() {
    ...
    vShininess = gl.getAttribLocation(program, "vShininess"); // Assuming you have shininess as a vertex attribute
    ambLight = gl.getUniformLocation(program, "ambLight");
    spotlightPos = gl.getUniformLocation(program, "spotlightPos");
    spotlightColor = gl.getUniformLocation(program, "spotlightColor");
    spotlightDirection = gl.getUniformLocation(program, "spotlightDirection");
    redLightPos = gl.getUniformLocation(program, "redLightPos");
    redLightColor = gl.getUniformLocation(program, "redLightColor");
    greenLightPos = gl.getUniformLocation(program, "greenLightPos");
    greenLightColor = gl.getUniformLocation(program, "greenLightColor");
    whiteLightPos = gl.getUniformLocation(program, "whiteLightPos");
    whiteLightColor = gl.getUniformLocation(program, "whiteLightColor");
    orangeLight1Pos = gl.getUniformLocation(program, "orangeLight1Pos");
    orangeLight1Color = gl.getUniformLocation(program, "orangeLight1Color");
    orangeLight1Direction = gl.getUniformLocation(program, "orangeLight1Direction");
    orangeLight2Pos = gl.getUniformLocation(program, "orangeLight2Pos");
    orangeLight2Color = gl.getUniformLocation(program, "orangeLight2Color");
    orangeLight2Direction = gl.getUniformLocation(program, "orangeLight2Direction");
    SPOTLIGHT = gl.getUniformLocation(program, "SPOTLIGHT");
    FLASHING = gl.getUniformLocation(program, "FLASHING");
    time = gl.getUniformLocation(program, "time");
    gl.uniform1i(SPOTLIGHT, 1);
...
function setVertexAttributes() {
    ...
    vNormal = gl.getAttribLocation(program, "vNormal");
    gl.vertexAttribPointer(vNormal, 3, gl.FLOAT, false, 48, 32);
...
function makeWaterAndBuffer() {
    let waterPoints = [];
    // Colors
    let blue = new vec4(0.5, 0.5, 1.0, 1.0);
    let normal = new vec3(0.0, 1.0, 0.0); //
    // Front top-left
    waterPoints.push(new vec4(-3.0, -0.1, 2.0, 1.0), blue, normal);
    // Front top-right
    waterPoints.push(new vec4(3.0, -0.1, 2.0, 1.0), blue, normal);
    // Back top-left
    waterPoints.push(new vec4(-3.0, -0.1, -2.0, 1.0), blue, normal);
    ...
    for (let px of positionsX) {
        for (let pz of positionsZ) {
            ...
            for (let i = 0; i < subdivisions; i++) {
                ...
                const x1 = px + poleRadius * Math.cos(angle);
                const z1 = pz + poleRadius * Math.sin(angle);
                const x2 = px + poleRadius * Math.cos(nextAngle);
                const z2 = pz + poleRadius * Math.sin(nextAngle);
                // Bottom cap
                wallPoints.push(new vec4(px, -0.1, pz, 1.0), poleColor);
                wallPoints.push(new vec4(x1, -0.1, z1, 1.0), poleColor);
                wallPoints.push(new vec4(x2, -0.1, z2, 1.0), poleColor);
                // Top cap
                wallPoints.push(new vec4(px, poleHeight, pz, 1.0), poleColor);
                wallPoints.push(new vec4(x1, poleHeight, z1, 1.0), poleColor);
                wallPoints.push(new vec4(x2, poleHeight, z2, 1.0), poleColor);
                // Side
                wallPoints.push(new vec4(x1, -0.1, z1, 1.0), poleColor);
                wallPoints.push(new vec4(x2, -0.1, z2, 1.0), poleColor);
                wallPoints.push(new vec4(x1, poleHeight, z1, 1.0), poleColor);
